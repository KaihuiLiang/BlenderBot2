...
from .web_retriever import GoogleRetriever
...
class BlenderBot2RagModel(RagModel):
    ...
    def __init__(self, opt: Opt, dictionary: DictionaryAgent, retriever_shared=None):
        ...
        query_encoder = (self.retriever.query_encoder if hasattr(self.retriever, 'query_encoder') and
                         opt['share_search_and_memory_query_encoder'] else None)
        ...
        # attrs
        self.knowledge_access_method = KnowledgeAccessMethod(opt['knowledge_access_method'])
        self.search = RetrieverType(opt['rag_retriever_type']) in [
            RetrieverType.SEARCH_ENGINE,
            RetrieverType.SEARCH_TERM_FAISS,
        ]
        self.should_generate_query = (self.knowledge_access_method is KnowledgeAccessMethod.CLASSIFY or self.search)
    ...
    ) -> Tuple[torch.Tensor, torch.BoolTensor, Optional[torch.LongTensor], Optional[List[List[Document]]],
    ...
    def get_retrieval_indices(self, ret_vec: torch.LongTensor, ret_type: RetrievalType) -> torch.LongTensor:
        """
    ...
    def retrieve_and_concat(
        ...
        n_input = (input_turns_cnt.sum().item() if input_turns_cnt is not None else input.size(0))
        # 0a. Classify retrieval type, if necessary
        generated_memories = [[] for _ in range(int(n_input))]
        if memory_decoder_vec is not None:
            generated_memories = self.memory_decoder.generate_memories(memory_decoder_vec, num_memory_decoder_vecs)
        ...
        search_indices = self.get_retrieval_indices(retrieval_type, RetrievalType.SEARCH)
        if search_indices.numel() > 0:
            search_docs, search_doc_scores = self.perform_search(search_queries, query_vec, search_indices)
        ...
        # 1b. memory search
        memory_indices = self.get_retrieval_indices(retrieval_type, RetrievalType.MEMORY)
        if memory_indices.numel() > 0:
            ...
            if memories is not None and memory_scores is not None:
                self._fill_docs_and_scores(top_docs, doc_scores, memory_indices, memories, memory_scores)

        # 1c. no search
        no_search_indices = self.get_retrieval_indices(retrieval_type, RetrievalType.NONE)
        if no_search_indices.numel() > 0:
            ...
            self._fill_docs_and_scores(top_docs, doc_scores, no_search_indices, dummy_docs, dummy_scores)
        ...
        expanded_input = self.concat_docs_and_input(input, input_lengths, top_docs, top_doc_scores.size(1))
            ...
            if num_gold is not None and num_gold[i] > 0:
                ...
                for g_idx, replace in enumerate(replace_inds):
                    ...
                    docs[idx][replace] = Document(
                        title=self.dict.vec2txt(clean_vec_with_dict(self.dict, gold_doc_titles[i][g_idx])),
                        text=self.dict.vec2txt(clean_vec_with_dict(self.dict, gold_docs[i][g_idx])),
                        docid='',
                    )
    ...
    def perform_search(
        ...
        if self.search:
            ...
            assert isinstance(self.retriever, SearchQuerySearchEngineRetriever) or isinstance(
                self.retriever, SearchQueryFAISSIndexRetriever)
    ...
    def access_long_term_memory(
        ...
        if memory_vec is not None:
            memory_dict = {
                batch_id: memory_vec[batch_id, :num_memories[mem_id]] for batch_id, mem_id in enumerate(indices)
            }
        if memory_decoder_vec is not None:
            for batch_id in indices:
                ...
                if not new_mems_i:
                    continue
                tokenized = [self.long_term_memory.tokenize_query(m) for m in generated_memories[batch_id]]
            ...
            for batch_id in memory_dict:
                ...
                if vec.size(-1) < max_length:
                    memory_dict[batch_id] = torch.cat(
                        [
                            vec,
                            torch.zeros((*vec.shape[:-1], max_length - vec.size(-1))).fill_(
...
class LongTermMemory(RagRetriever):
    ...
    def __init__(
        ...
        self._tokenizer = RagRetrieverTokenizer(
            ...
            if opt['memory_retriever_truncate'] > 0 else opt['rag_query_truncate'],
        )
        ...
        self.memory_vec_dict: Dict[int, torch.LongTensor] = {  # type: ignore
            k: torch.zeros(self.max_memories, opt['max_doc_token_length']).to(torch.int64)
            for k in range(self.num_memory_slots)
        }
        self.memory_enc_dict: Dict[int, torch.Tensor] = {
            k: torch.zeros(self.max_memories, opt['retriever_embedding_size']) for k in range(self.num_memory_slots)
        }
    ...
    def score_memories(self, query_enc: torch.Tensor) -> List[torch.Tensor]:
        ...
        for i in range(query_enc.size(0)):
            scores.append((query_enc[i:i + 1] @ self.memory_enc_dict[self.active_memory_slots[i]].t()).squeeze(0))
        return scores
...
class BlenderBot2FidModelMixin:
    ...
    def encoder(
        ...
        new_out, new_mask = concat_enc_outs(input, enc_out, mask, self.embedding_size, self.pad_idx)
...
class T5BlenderBot2FidModel(BlenderBot2FidModelMixin, T5FidModel, T5BlenderBot2RagModel):
    pass
...
class BB2SearchQuerySearchEngineRetriever(BB2SearchRetrieverMixin, SearchQuerySearchEngineRetriever):
    """
...
class BB2SearchQueryFaissIndexRetriever(BB2SearchRetrieverMixin, SearchQueryFAISSIndexRetriever):
    """
    Override Search Engine Retriever to accomodate SQ Generator from BB2 Setup.
    """